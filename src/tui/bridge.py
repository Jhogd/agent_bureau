...
_SHARED_PREAMBLE = """\
You are one of two AI agents in Agent Bureau. Both agents receive the same \
user prompt simultaneously and respond independently. Answer the task directly \
— do not simulate, anticipate, or describe any subsequent steps.

## Output format
- Produce working, complete code — not pseudocode.
- Wrap code in fenced blocks with a language tag:
    ```python
    <code>
    ```
- If the code targets a specific file, add the path as a comment on the \
...
async def _stream_pipe(
    ...
    proc = await asyncio.create_subprocess_exec(
        *spec.build_argv(prompt),
        ...
        stderr=asyncio.subprocess.DEVNULL,  # discard stderr; status/progress noise from agents
    )
